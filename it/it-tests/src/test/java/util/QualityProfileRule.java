...
public class QualityProfileRule extends ExternalResource implements QualityProfileSupport {
  ...
  /**
   * Enables the automatic deletion of custom quality profiles after
   * each test or test class.
   *
   * By default custom Quality profiles are not deleted by this JUnit
   * rule because deletion is supposed to be already handled by
   * {@link OrganizationRule} when organizations are enabled.
   */
  public QualityProfileRule enableAutoCleanup() {
  ...
  @Override
  public QualityProfileSupport delete(QualityProfile profile) {
    return support.delete(profile);
  }

  @Override
  public QualityProfileSupport activateRule(QualityProfile profile, String ruleKey) {
    return support.activateRule(profile, ruleKey);
  }

  @Override
  public QualityProfileSupport assertThatNumberOfActiveRulesEqualsTo(QualityProfile profile, int expectedActiveRules) {
    return support.assertThatNumberOfActiveRulesEqualsTo(profile, expectedActiveRules);
  }
  ...
  private static class QualityProfileSupportImpl implements QualityProfileSupport {
    ...
    public QualityProfileSupportImpl(WsClient wsClient) {
    ...
    @Override
    public QualityProfileSupport delete(QualityProfile profile) {
      getWsService().delete(new DeleteRequest(profile.getKey()));
      return this;
    }
