...
function getCommand1(token: string, host: string, projectKey: string, organization?: string) {
  ...
  return `SonarQube.Scanner.MSBuild.exe begin${s}/k:"${projectKey}"${s}${organizationPart}/d:"sonar.cfamily.build-wrapper-output=bw-output"${s}/d:"sonar.host.url=${host}${s}/d:"sonar.login=${token}"`;
  /* eslint-enable max-len */
}

function getCommand2() {
  return 'build-wrapper-win-x86-64.exe --out-dir bw-output MsBuild.exe /t:Rebuild';
}
...
export default class Msvc extends React.PureComponent {
  props: Props;
